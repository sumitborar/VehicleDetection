...
# from Udacity L22 lesson_functions.py
...
def conv_colorspace( image, color_space ):
    if color_space != 'RGB':
        if color_space == 'HSV':
            feature_image = cv2.cvtColor(image, cv2.COLOR_RGB2HSV)
        elif color_space == 'LUV':
...
## A sliding window function slide_window to generate a list of boxes with predefined parameters
...
def single_img_features(img, color_space='RGB', spatial_size=(32, 32),
    ...
    if spatial_feat == True:
        ...
        #print (spatial_features.shape, "Spatial feature shape ")
        img_features.append(spatial_features)
    #5) Compute histogram features if flag is set
    if hist_feat == True:
        hist_features = color_hist(feature_image, nbins=hist_bins)
        #6) Append features to list
        #print (hist_features.shape, "Hist feature shape ")
...
## a function to draw the list of boxes on an image
